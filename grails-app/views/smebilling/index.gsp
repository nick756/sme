            ...
            <g:if test="${instancesList.size() > 0}">
                <g:each in="${instancesList}" status="index" var="objectInstance">
                    ...
                    <tr class="${(index % 2) == 0 ? 'even' : 'odd'}">
                        ...
                        <td class="no-border">${objectInstance?.paid}</td>
                        <td class="no-border centered">
                            <g:link action="print" params="['max': params.max, 'offset': params.offset, 'filterDateFrom': params?.filterDateFrom, 'filterDateTill': params?.filterDateTill, 'filterOperation.id': params?.filterOperation?.id]">
                                <img src="${resource(dir: 'images', file: 'icons/printer01_16x16.png')}" title="<g:message code='actions.bill.print'/>"/>
                            </g:link>
                            <g:link action="edit" params="['max': params.max, 'offset': params.offset, 'filterDateFrom': params?.filterDateFrom, 'filterDateTill': params?.filterDateTill, 'filterOperation.id': params?.filterOperation?.id]">
